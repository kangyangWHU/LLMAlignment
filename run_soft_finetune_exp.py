...
if __name__ == "__main__":
    ...
    sft_type = "soft_sft"
    r = 8
    for llm_name in [LLM_Name.qwen_7b, LLM_Name.llama2_7b]:
        ...
        for task_dataset in [TaskName.toxicity]:
            ...
            for n_harmful in n_harmful_list:
                ...
                if sft_type == "soft_sft":
                    output_dir = "../softsft_outputs/{}/{}/r{}/{}/harmful{}".format(llm_name, task_dataset,str(r), harmful_name, n_harmful)
                else:
                    output_dir = "../sft_outputs/{}/{}/r{}/{}/harmful{}".format(llm_name, task_dataset,str(r), harmful_name, n_harmful)

                with open(cfg_path, 'r') as file:
                    config = yaml.safe_load(file)

                # Modify the config
                config['sft_type'] = sft_type
                ...
                # soft sft needs smaller batch size
                config["per_device_train_batch_size"] = 48

                # gemma take more GPU
                if llm_name in [LLM_Name.gemma_2b, LLM_Name.qwen_7b]:
                    config['per_device_train_batch_size'] = config['per_device_train_batch_size']//3
                elif llm_name == LLM_Name.llama2_13b:
                ...
                # train the model
                seed_everything(42)
                command = ["python", "soft_finetune.py", "--path", new_cfg]
                run_command(command)
